...
function updateCarPositions(cars) {
    ...
    cars.forEach(car => {
        ...
        // Create popup content with named passengers highlighted
        let passengersHtml = '';
        let hasNamedPassengers = false;
        
        if (car.passengers && car.passengers.length > 0) {
            passengersHtml = '<div class="passengers-list">';
            car.passengers.forEach(p => {
                // Highlight named passengers (custom bookings) with a different style
                if (p.name) {
                    hasNamedPassengers = true;
                    passengersHtml += `<div class="named-passenger"><strong>${p.name}</strong>: ${p.pickup} → ${p.dropoff}</div>`;
                } else {
                    passengersHtml += `<div>${p.id}: ${p.pickup} → ${p.dropoff}</div>`;
                }
            });
            passengersHtml += '</div>';
        }
        ...
        // Add passenger name labels near car if they are custom bookings
        if (car.passengers && car.passengers.length > 0) {
            // Count named passengers to position multiple names
            let namedPassengerCount = 0;
            
            car.passengers.forEach(passenger => {
                if (passenger.name) {
                    // Create a name tag that follows the car
                    const nameTag = L.marker(coords, {
                        icon: L.divIcon({
                            className: 'passenger-name-tag',
                            html: passenger.name,
                            iconSize: [80, 24],
                            iconAnchor: [40, 10 + (namedPassengerCount * 20)] // Stack names vertically
                        }),
                        zIndexOffset: 900
                    }).addTo(map);
                    
                    // Store this name tag with the car marker for removal later
                    carMarkers[`${car.id}-name-${passenger.name}`] = nameTag;
                    
                    // Increment counter for vertical positioning
                    namedPassengerCount++;
                }
            });
        }
...
function updateRequestMarkers(requests) {
    ...
    requests.forEach(request => {
        ...
        // Check if this is a named request (custom booking)
        const isNamedRequest = request.name && request.name.trim() !== '';
        
        let requestIcon;
        
        if (isNamedRequest) {
            // Create a custom icon with the passenger's name
            requestIcon = L.divIcon({
                className: 'named-request-marker',
                html: `<div class="named-request-icon">${request.name}</div>`,
                iconSize: [80, 24],  // Make wider to accommodate names
                iconAnchor: [40, 12]  // Center the icon
            });
        } else {
            // Use the standard request icon
            requestIcon = L.divIcon({
        ...
        const marker = L.marker(coords, {
            icon: requestIcon,
            title: isNamedRequest ? 
                `${request.name}: ${request.pickup} → ${request.dropoff}` :
                `Request: ${request.pickup} → ${request.dropoff}`,
            zIndexOffset: 900
        }).addTo(map);
        
        // Enhanced popup with different styling for named requests
        if (isNamedRequest) {
            marker.bindPopup(`
                <div class="request-popup">
                    <strong style="color:#ff9800;">Custom Booking for ${request.name}</strong>
                    <div style="margin-top:5px;">From: ${request.pickup}</div>
        ...
        // For named requests, add a pulsing effect at the pickup location
        if (isNamedRequest) {
            ...
            // Store the pulse marker so we can remove it later
            requestMarkers[`${request.id}-pulse`] = pulse;
        }
    });
}
